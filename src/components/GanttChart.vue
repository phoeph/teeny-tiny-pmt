    ...
    <div class="gantt-container">
      ...
      <div class="gantt-body">
        ...
        <div class="priority-list">
          <div v-for="task in tasks" :key="'p-'+task.id" class="task-group-priority">
            <div class="priority-row">
              <span class="priority-badge">{{ getPriority(task) }}</span>
            </div>
            <div v-for="subtask in task.subtasks" :key="'p-'+subtask.id" class="priority-row">
              <span class="priority-badge">{{ getPriority(subtask) }}</span>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getPriority(item) {
      const p = item && item.priority
      if (p === undefined || p === null) return '中'
      if (typeof p === 'number') {
        if (p >= 3) return '高'
        if (p <= 1) return '低'
        return '中'
